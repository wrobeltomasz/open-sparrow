...
const NON_CONFIG_TABS = new Set(['users', 'security', 'health', 'backup', 'database', 'audit', 'add_table', 'performance', 'cron', 'm2m']);
...
async function loadConfigFile(fileName) {
    if (fileName === 'health' || fileName === 'docs' || fileName === 'users' || fileName === 'backup' || fileName === 'menu' || fileName === 'audit' || fileName === 'add_table' || fileName === 'migrations' || fileName === 'performance' || fileName === 'cron' || fileName === 'm2m') {
        currentConfig = null;
        renderSidebar();
...
function renderSidebar() {
    ...
    if (currentFile === 'database' || currentFile === 'security' || currentFile === 'health' || currentFile === 'docs' || currentFile === 'users' || currentFile === 'backup' || currentFile === 'menu' || currentFile === 'audit' || currentFile === 'add_table' || currentFile === 'migrations' || currentFile === 'performance' || currentFile === 'cron' || currentFile === 'm2m') {
...
function renderEditor(key, itemData, isArray) {
    ...
    if (['health', 'docs', 'users', 'backup', 'menu', 'audit', 'add_table', 'performance', 'cron', 'm2m'].includes(currentFile) || (currentFile === 'files' && key === 'MANAGER')) {
